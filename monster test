start(Ans) :-
    write("What attack level are you?: "),flush_output(current_output),
    readln([Head|Attlvl]),
	write("What magic level are you?: "),flush_output(current_output),
    readln([Head1|Magiclvl]),
	write("What range level are you?: "),flush_output(current_output),
    readln([Head2|Rangelvl]),
	write("Are you a member?: y/n "),flush_output(current_output),
    readln([Head3|Lnmem]),
	assert(user_attlevel(Head)),
	assert(user_magelevel(Head1)),
	assert(user_rangelevel(Head2)),
	assert(user_member(Head3)).
...
noun([monster | T],T,Obj) :- monster(Obj).
noun([monster,i,can,kill | T],T,Obj) :- monster_kill(Obj).
...
% ?- ask(['What',is,a,monster,i,can,kill],A).
